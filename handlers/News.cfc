...
component extends="cborm.models.EventHandler"{
	...
	function save(event,rc,prc) {
		...
		local.strLargeFileName = createObject("component", "includes.cfc.Utilities").processServerImage(
			...
			, strImageType = "news"
			, strUploadedImagesFolderLocation = application.stcApplicationCustomSettings.strUploadedNewsImagesFolderLocation
		);
	...
	function remove(event,rc,prc) {
		request.aryPost = entityLoadByPK("Post", val(arguments.rc.post_id));
		local.strFullLargeFilePath = expandPath(application.stcApplicationCustomSettings.strUploadedNewsImagesFolderLocation & request.aryPost.getPost_large_image_file_name_tx());
		local.strFullSmallFilePath = expandPath(application.stcApplicationCustomSettings.strUploadedNewsImagesFolderLocation & request.aryPost.getPost_small_image_file_name_tx());
